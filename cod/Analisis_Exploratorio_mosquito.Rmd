...
# Limpieza de datos
## Estimación de valores coordenadas
```{r}
# Cargando una librería para utilizar funciones específicas

#Arreglar columnas latitud y longitud a escala
data_clea <- data_ori

#Eliminar puntos de cols
data_clea$latitude <- gsub("\\.", "", data_clea$latitude)
data_clea$longitude <- gsub("\\.", "", data_clea$longitude)

#Hacerlas numéricas
data_clea$latitude <- as.numeric(data_clea$latitude)
data_clea$longitude <- as.numeric(data_clea$longitude)
data_clea$elevation <- as.numeric(data_clea$elevation)

#escalar columnas
data_clea$latitude <- round(data_clea$latitude / 1e6, 6)
data_clea$longitude <- round(data_clea$longitude / 1e6, 6)
...
```{r}
data_clea <- data_clea %>%
  mutate(`mosquito habitat mapper:water source` = recode(`mosquito habitat mapper:water source`, 
...
data_clea <- data_clea %>%
  mutate(`mosquito habitat mapper:water source type` = recode(`mosquito habitat mapper:water source type`, 
...
data_clea$`mosquito habitat mapper:water source type` <- as.factor(data_clea$`mosquito habitat mapper:water source type`)

ggplot(data_clea, aes(x = `mosquito habitat mapper:water source type`)) + 
...
```{r, echo=FALSE}
ggplot(data_clea, aes(x = `mosquito habitat mapper:water source`, fill = `mosquito habitat mapper:water source`)) + 
...
data_clea$`mosquito habitat mapper:mosquito pupae` <- as.factor(data_clea$`mosquito habitat mapper:mosquito pupae`)

# Crear un gráfico de barras 
ggplot(data_clea, aes(x = `mosquito habitat mapper:mosquito pupae`, fill = `mosquito habitat mapper:mosquito pupae`)) + 
...
data_clea$`mosquito habitat mapper:mosquito eggs` <- as.factor(data_clea$`mosquito habitat mapper:mosquito eggs`)

# Crear un gráfico de barras 
ggplot(data_clea, aes(x = `mosquito habitat mapper:mosquito eggs`, fill = `mosquito habitat mapper:mosquito eggs`)) + 
  geom_bar(color = "black", width = 0.7) +  # Añadir borde negro y ajustar el ancho de las barras
  geom_text(stat='count', aes(label = ..count..), position = position_stack(vjust = 0.5), color = "white", size = 5) +  # Agregar etiquetas centradas
...
## Gráfico de barra sobre la variable mosquitos adultos `mosquito habitat mapper:mosquito adults`
```{r, echo=FALSE}
#Hacer de que la columna sea un factor
data_clea$`mosquito habitat mapper:mosquito adults` <- as.factor(data_clea$`mosquito habitat mapper:mosquito adults`)

# Crear un gráfico de barras
ggplot(data_clea, aes(x = `mosquito habitat mapper:mosquito adults`, fill = `mosquito habitat mapper:mosquito adults`)) + 
  geom_bar(color = "black", width = 0.7) +  # Añadir borde negro y ajustar el ancho de las barras
  geom_text(stat='count', aes(label = ..count..), position = position_stack(vjust = 0.5), color = "white", size = 5) +  # Agregar etiquetas centradas
...
ggplot(data_clea, aes(x = `mosquito habitat mapper:genus`)) +
...
ggplot(data_clea, aes(x = reorder(`mosquito habitat mapper:species`, `mosquito habitat mapper:species`, function(x)-length(x)))) +
